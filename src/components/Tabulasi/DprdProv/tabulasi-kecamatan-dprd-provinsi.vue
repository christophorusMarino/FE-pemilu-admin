<template>
    <div class="ma-5">
      <v-row no-gutters>
        ...
        <v-col cols="3">
          <v-autocomplete
            ...
            dense
            outlined
            :items="listDapilDprdProvKec"
            item-text="nama"
            item-value="nama"
            @change="tabulasiDprdProvKec"
          ></v-autocomplete>
        </v-col>
        <v-col cols="1">
          <v-card-title class="py-0 justify-center" v-if="dapil">
            <v-icon x-large @click="tabulasiDprdProvKec">
              mdi-refresh-circle
            </v-icon>
          </v-card-title>
        </v-col>
      ...
      <v-row no-gutters v-if="!loading">
        <v-col cols="12" v-for="(data, idx) in dataHasilDprdProvKec" :key="idx">
          <v-card class="ma-2">
            <v-row no-gutters>
              <v-col cols="12">
                <v-card-title
              ...
              <v-row no-gutters>
                <v-col cols="6">
                  ...
                  <v-card-text class="ma-0 px-2">
                    <Bar
                      ...
                      :data="resultPartaiDprdProvKec[idx].result"
                      :chart-id="chartId"
                ...
                <v-col cols="6">
                  ...
                  <v-card-text class="ma-0 px-2">
                    <Bar
                      :chart-options="chartOptions"
                      :data="resultCalegDprdProvKec[idx].result"
                      :chart-id="chartId"
                      :dataset-id-key="datasetIdKey"
                      :width="width"
                      :height="height"
                ...
                <v-col cols="12">
                  ...
                  <v-card-text class="ma-0 px-2">
                    <v-row no-gutters>
                      <v-col
                        cols="2"
                        v-for="(val, key, index) in dataHasilDprdProvKec[idx]
                          .kursi"
                        :key="index"
                      >
  ...
  export default {
    ...
    data: () => ({
      ...
      textSnackbar: "",
      colorSnackbar: "",
      listDapilDprdProvKec: [],
      dataHasilDprdProvKec: [],
      dataHasilCalegDprdProvKec: [],
      chartOptions: {
        responsive: true,
      ...
      resultPartaiDprdProvKec: [],
      resultCalegDprdProvKec: [],
      fromKecDprdProv: [],
    }),
  
    watch: {
      layer() {
        if (this.layer === "DPRD_PROV" && this.jenis === "KECAMATAN_DPR_RI") {
          if (this.dapil) {
    ...
    methods: {
      ...
      async tabulasiDprdProvKec() {
        this.resultPartaiDprdProvKec = [];
        this.resultCalegDprdProvKec = [];
        this.loading = true;
        ...
        await this.getTabulasi(param)
          .then((response) => {
            this.dataHasilDprdProvKec = response.hasilFinal;
        ...
        let uid_dapil = this.listDapilDprdProvKec.find(
          (el) => el.nama === this.dapil
        ).uid;
        this.getSuara(uid_dapil).then((response) => {
          this.fromKecDprdProv = response.data;
        });
      },
      setDataHasilPartai(data) {
        data.forEach((el) => {
          ...
          this.resultPartaiDprdProvKec.push(rest);
        });
        this.loading = false;
      },
      setHasilCaleg(data) {
        data.forEach((el) => {
          ...
          this.resultCalegDprdProvKec.push(rest);
        });
        this.loading = false;
      },
    },
    ...
    mounted() {
      ...
      this.getDapil(payload).then((response) => {
        this.listDapilDprdProvKec = response.data;
      });
      if (this.layer === "DPRD_PROV" && this.jenis === "KECAMATAN_DPR_RI") {
        if (this.dapil) {
          this.tabulasiDprdProvKec();
        }
      }
    },
  };
