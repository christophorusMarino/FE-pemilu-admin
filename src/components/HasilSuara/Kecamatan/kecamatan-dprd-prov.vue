<template>
  <div>
    <v-card flat v-if="kec">
      <template v-if="dataDapilDprdProv?.partai">
...
export default {
  ...
  methods: {
    ...mapActions({
      ...
      postDataDprdProv: "hasil/postHasilSuaraKecamatan",
    }),
    async getDapil() {
      let queryDapil = {
    ...
    async postHasil() {
      ...
      dataSend.append("jenis", "DPRD PROVINSI RIAU");
      ...
      if (this.fotoDprdProv.length > 0) {
        ...
        this.fotoDprdProv.forEach((item, index) => {
          let kode = `C1-${this.kabupaten}-${this.kecamatan}-CAMAT-${
            index + 1
          }-${timeStamp}-DPRDPROVINSI.${item.file.name.split(".").pop()}`;
          console.log(kode);
          newFoto.push(
    ...
    async post(data) {
      await this.postDataDprdProv(data)
        .then((response) => {
